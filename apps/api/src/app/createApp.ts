  ...
  app.use(cors({
    origin: (origin, cb) => {
      ...
      try {
        ...
        const allowedLocalhost = /^http:\/\/localhost:(5173|5174|3000|3001|4173)$/;
        // Permitir IPs da rede local em portas de desenvolvimento (mais flexível)
        const allowedLan = /^http:\/\/(192\.168\.[0-9]+\.[0-9]+|10\.[0-9]+\.[0-9]+\.[0-9]+|172\.(1[6-9]|2[0-9]|3[0-1])\.[0-9]+\.[0-9]+):(5173|5174|3000|3001|4173)$/;
        // Permitir outros IPs comuns de desenvolvimento
        const allowedDev = /^http:\/\/(127\.0\.0\.1|0\.0\.0\.0):(5173|5174|3000|3001|4173)$/;
        // Permitir acesso de dispositivos móveis com portas padrão
        const allowedMobile = /^http:\/\/(192\.168\.[0-9]+\.[0-9]+|10\.[0-9]+\.[0-9]+\.[0-9]+):(5173|3000)$/;
