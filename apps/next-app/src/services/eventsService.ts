...
export async function getTimeline(prisma: PrismaClient, startDate: string, endDate: string, user_id?: string) {
...
export async function checkConflicts(prisma: PrismaClient, event_date: string, start_time: string, end_time: string, professional: string, excludeId?: string, user_id?: string) {
  const EFFECTIVE_USER_ID = user_id || MOCK_USER_ID;
  // Note: This would need to be adapted to work with Prisma instead of Pool
...
export async function createEvent(
  ...
  if (!isPastEvent) {
    ...
    // Simplified conflict check - would need full implementation
    const conflictsResult = await checkConflicts(prisma, event_date, start_time, end_time, professional, undefined, EFFECTIVE_USER_ID);
...
export async function getEventsByPeriod(prisma: PrismaClient, startDate: string, endDate: string, user_id?: string) {
  const EFFECTIVE_USER_ID = user_id || MOCK_USER_ID;
...
export async function updateEvent(prisma: PrismaClient, id: string, eventData: any, override_travel_conflict: boolean = false, user_id?: string) {
  const { type, professional, event_date, start_time, end_time, notes, treatment_total_doses, treatment_alert_threshold, stock_quantity, schedule_return_reminder = false } = eventData;

  const EFFECTIVE_USER_ID = user_id || MOCK_USER_ID;
  const conflictsResult = await checkConflicts(prisma, event_date, start_time, end_time, professional, id, EFFECTIVE_USER_ID);
  const { overlapConflicts, travelConflicts } = conflictsResult;
  if (overlapConflicts && overlapConflicts.length > 0) {
    throw { conflictType: 'overlap', conflicts: overlapConflicts, message: 'Existe sobreposição de horários com outro(s) evento(s).' };
  }
...
export async function getEventById(prisma: PrismaClient, id: string, user_id?: string) {
...
export async function deleteEvent(prisma: PrismaClient, id: string, user_id?: string) {
  const EFFECTIVE_USER_ID = user_id || MOCK_USER_ID;

  // Mark files as orphans
  await prisma.event_files.updateMany({
    where: { event_id: id },
    data: { is_orphan: true, orphaned_at: new Date() }
  });

  const updateRes = await prisma.events.updateMany({
    where: { id, user_id: EFFECTIVE_USER_ID, deleted_at: null },
...
export async function generateUploadCode(prisma: PrismaClient, id: string, fileType: string, user_id?: string) {
  const EFFECTIVE_USER_ID = user_id || MOCK_USER_ID;
