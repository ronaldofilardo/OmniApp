...
export async function getFilesForEvent(prisma: PrismaClient, eventId: string, token?: string) {
  ...
  return files.map((file: any) => {
    ...
    const baseUrl = `${process.env.BASE_URL || 'http://localhost:3000'}/api/files/${file.id}/view`;
    return {
      ...file,
      url: baseUrl
    };
...
export async function uploadFile(prisma: PrismaClient, eventId: string, file: any, fileType: string) {
...
export async function uploadFileByCode(prisma: PrismaClient, uploadCode: string, file: any) {
