...
export async function getTimeline(prisma: PrismaClient, startDate: string, endDate: string) {
  // Lógica de getTimeline
  const eventsResult = await prisma.events.findMany({
    where: { user_id: MOCK_USER_ID, deleted_at: null }
  });
  const eventMasters = eventsResult;
...
export async function checkConflicts(prisma: PrismaClient, event_date: string, start_time: string, end_time: string, professional: string, excludeId?: string) {
  return await detectConflicts(prisma, MOCK_USER_ID, { event_date, start_time, end_time, professional, excludeId });
}

export async function createEvent(prisma: PrismaClient, eventData: any, override_travel_conflict: boolean = false) {
  ...
  if (!isPastEvent) {
    ...
    const conflictsResult = await detectConflicts(prisma, MOCK_USER_ID, { event_date, start_time, end_time, professional });
    ...
    if (travelConflicts && travelConflicts.length > 0 && TRAVEL_GAP_ENABLED) {
      ...
      try {
        await prisma.event_conflict_overrides.create({
          data: {
            user_id: MOCK_USER_ID,
            event_id: null,
            overridden_conflicts: JSON.stringify(travelConflicts)
          }
        });
      } catch (err) {
...
export async function getEventsByPeriod(prisma: PrismaClient, startDate: string, endDate: string) {
...
export async function updateEvent(prisma: PrismaClient, id: string, eventData: any, override_travel_conflict: boolean = false) {
  const { type, professional, event_date, start_time, end_time, notes, treatment_total_doses, treatment_alert_threshold, stock_quantity, schedule_return_reminder = false } = eventData;

  const conflictsResult = await detectConflicts(prisma, MOCK_USER_ID, { event_date, start_time, end_time, professional, excludeId: id });
  if (conflictsResult.error === 'invalid_datetime') {
    throw new Error('Datas/horários inválidos.');
  }
  const { overlapConflicts, travelConflicts } = conflictsResult;
...
export async function getEventById(prisma: PrismaClient, id: string) {
  ...
  const event = await prisma.events.findFirst({
    where: { id, user_id: MOCK_USER_ID, deleted_at: null }
  });
...
export async function deleteEvent(prisma: PrismaClient, id: string) {
  ...
  if (!event) throw new Error('Evento não encontrado ou já deletado.');

  // Marcar arquivos como órfãos
  await prisma.event_files.updateMany({
    where: { event_id: id },
    data: { is_orphan: true, orphaned_at: new Date() }
  });

  // Marcar evento como deletado
...
export async function generateUploadCode(prisma: PrismaClient, id: string, fileType: string) {
...
export async function getUploadCodeForEvent(prisma: PrismaClient, id: string, fileType: string) {
...
export async function getUploadCodesForEvent(prisma: PrismaClient, id: string) {
